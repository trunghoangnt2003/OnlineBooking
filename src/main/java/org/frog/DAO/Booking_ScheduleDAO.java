...
public class Booking_ScheduleDAO {
    ...
    public ArrayList<Booking> getBookingsHistory(String id) {
        ...
        try {
            ...
            String sql = "  Select  b.id,b.status_id,status.type,\n" +
                    ...
                    "\t\t\t\t\tINNER JOIN Account acc ON acc.id = m.account_id\n" +
                    "\t\t\t\t\tINNER JOIN Status status ON status.id = b.status_id\n" +
                    "WHERE b.mentor_id = ? AND b.status_id = 2 OR b.status_id = 13 AND b.mentor_id = ? ";
            PreparedStatement preparedStatement = connection.prepareStatement(sql);
            preparedStatement.setString(1, id);
    ...
    public ArrayList<Booking> filterBookingsHistoryByStatus(String id, String name, int statusId, String start, String end) {
        ArrayList<Booking> bookings = new ArrayList<>();
        int count = 2;
        try {
            Connection connection = JDBC.getConnection();
            ...
            if(!start.isEmpty()){
                count++;
              preparedStatement.setDate(count, Date.valueOf(start));
            }
            if (!end.isEmpty()) {
                count++;
                preparedStatement.setDate(count, Date.valueOf(end));
            }
            ResultSet resultSet = preparedStatement.executeQuery();
            while (resultSet.next()) {
