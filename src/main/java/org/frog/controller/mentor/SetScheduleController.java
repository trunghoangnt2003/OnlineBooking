                ...
                for(Week week : weeks){
                    ...
                    if(slotInfo[0].equals(week.getDayOfWeek())){
                        for (int i = 1; i < slotInfo.length; i++) {
                            numberUpdateSuccess += scheduleDAO.insertDayFreeByMentor(account.getId(),DateTimeHelper.convertStringToDateByDay(week.getDayOfMonth()),Integer.parseInt(slotInfo[i]));
                        }
                    }
                }
            }
